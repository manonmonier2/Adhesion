...
# defintion de la limite de temps pour un pic de bruit
time_limit = 3
...
for(id in list_id){
  ...
  # get experience time
  exp_time = max(data$time)#dur?e de l'exp
...
file_path = paste(path_output, "/index.csv", sep = "")
write.table(df, file = file_path, quote = F, col.names = T, row.names = F, sep = "\t")
